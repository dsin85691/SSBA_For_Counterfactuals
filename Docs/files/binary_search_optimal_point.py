...
from .common_functions import get_multi_dim_border_points, det_constraints, real_world_constraints
from .common_functions import balance_dataset, check_class_balance, convert_columns, gaussian_sample_generator_gpu
...
def batch_gaussian_samples(correct_dataset, label_vector, model, cov_matrix, columns, threshold, batch_size):
    correct_dataset = deepcopy(correct_dataset)
    for batch in batch_iter(zip(correct_dataset, label_vector), batch_size=batch_size):
        samples, labels = zip(*batch)
        correct_dataset = gaussian_sample_generator_gpu(samples=samples, dataset=correct_dataset, classifier=model,
                                cov_matrix=cov_matrix, columns=columns, labels=labels,
                                size=batch_size)
        if correct_dataset.shape[0] == threshold:
...
def multi_decision_boundary(model, X, y, epsilon=1e-3, threshold=10000, batch=1000):
    ...
    total_size = correct_a.shape[0] * correct_b.shape[0]

    if threshold < total_size:
        cov_matrix = np.cov(X_np, rowvar=False)

        label_a_vector = np.array([label_a] * correct_a.shape[0])
        label_b_vector = np.array([label_b] * correct_b.shape[0])

        correct_a = batch_gaussian_samples(correct_a, label_a_vector, model, cov_matrix, columns=X.columns, threshold=threshold, batch_size=batch)
        correct_b = batch_gaussian_samples(correct_b, label_b_vector, model, cov_matrix, columns=X.columns, threshold=threshold, batch_size=batch)
    ...
    num_pairs = min(threshold, total_size)  # Avoid overflow
    a_indices = np.random.choice(correct_a.shape[0], num_pairs, replace=True)
    b_indices = np.random.choice(correct_b.shape[0], num_pairs, replace=True)
